...
- 将高光项分母项中的 $\text{cos}\theta_i$ 去掉，因为当 $\theta_i$ 趋近于90°的时候，BRDF 趋近无穷了，这样在最终的着色结果中很有可能在 $90\degree$ 的时候出现颜色的突变。对于原计算公式来说，多乘了一个 $\text{cos}\theta_i$ ，即考虑到了光线倾斜照射时辐照度的减弱，这其实是更符合物理规律的。
...
用 $\hat h=\frac{\hat v+\hat l}{|\hat v+\hat l|}$ 和 $\hat n$ 的内积 $\text{cos}\beta$ 代替 $\hat v$ 和 $\hat r$ 的内积 $\cos\alpha_r$ 。其他与 phong 模型相同。未 normalized 的 BRDF 为：
$$
